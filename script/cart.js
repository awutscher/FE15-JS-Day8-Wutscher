const products = [
  {
    name: "Rose",
    image: "rose.jpg",
    cardText: "Fitting for every Anniversary !",
    price: 2.30,
    qtty: 1,
  },
  {
    name: "Lavender",
    image: "lavender.jpg",
    cardText: "Gives of some wonderful Aroma !",
    price: 1.50,
    qtty: 1,
  },
  {
...
for (let product of products) {
  ...
  <div class="card product col my-4" style="width: 300px;">
                <img class="card-img-top mt-2 p-4" src="./images/${
                  product.image
                }" alt="${product.name}">
...
function discount(total){
  ...
  if(total > 100){
    console.log(total)
    discountDiv.classList.replace("discountNotApplied", "discountApplied");
    discountDiv.classList.replace("textBlack","textWhite");
    document.getElementById("price").innerHTML = currencyFormater.format(total - (total * .1));
  }
  else{
    discountDiv.classList.replace("discountApplied", "discountNotApplied");
    discountDiv.classList.replace("textWhite","textBlack");
  }
  /* console.log(cart) */
};
...
function totalAmountItems(){
  let totalAmountDiv = document.querySelector("#totalAmount");
  let totalAmount = cart.reduce((sum, object) => {
    return sum + object.qtty;/* sum = startvalue , "+object.qtty" always adds qtty value to the sum / 
    works like this qtty1 + qtty2 + qtty3 */
